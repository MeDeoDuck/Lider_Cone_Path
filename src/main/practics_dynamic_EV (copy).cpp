...
private:
    ...
    ros::Subscriber subLeftConesCloud;
    ros::Subscriber subRightConesCloud;
...
public:
    ...
    ImageProjection() : nh("~") {
        ...
        subLeftConesCloud = nh.subscribe("/Left_cones_Cloud", 10, &ImageProjection::leftConeCallback, this);
        subRightConesCloud = nh.subscribe("/Right_cones_Cloud", 10, &ImageProjection::rightConeCallback, this);
    ...
    void leftConeCallback(const sensor_msgs::PointCloud2::ConstPtr& msg) {
        pcl::PointCloud<PointType>::Ptr leftCloud(new pcl::PointCloud<PointType>());
        pcl::fromROSMsg(*msg, *leftCloud);

        // 추후에 사용을 위해 멤버 변수로 저장하거나 바로 identifyLRcone에 사용 가능
        latestLeftCloud = leftCloud;  // 예: 멤버 변수로 저장
        }
    void rightConeCallback(const sensor_msgs::PointCloud2::ConstPtr& msg) {
        pcl::PointCloud<PointType>::Ptr rightCloud(new pcl::PointCloud<PointType>());
        pcl::fromROSMsg(*msg, *rightCloud);

        latestRightCloud = rightCloud;
    }
    ...
    void getPointCloud() {
        ...
        // 관심 영역 설정
        pointCloudGenerator.getInterestCloud(fullCloud, interestCloud, {-1, 3}, {-4, 4}, {0.3, 2});
        ...
        // 콘(작은 객체) 인식
        pointCloudGenerator.getAngleCloud(smallObject_Cloud, coneClusterCloud, {-1, 5}, {-3, 3}, {0.3, 2}, {-140, 140})
        ;
        ...
        if (preprocessor.marker_is_in_area(carBboxArray, {0, 4.5}, {-1, 1}))
            new_emergency = true;
        pointCloudGenerator.getInterestCloud(smallObject_Cloud, fovCloud, {0.3, 1.8}, {-0.6, 0.6}, {0.3, 2});
        ...
        // Path planning 관련 부분 (필요 시 주석 해제)
        // pathPlaner.getAPFAngle(coneClusterCloud, angleMarkerArray, 6, 0.5, true);
        // 클러스터링 vector 생성 (필요 시 주석 해제)
        // pointCloudGenerator.getObjectClusterCloud(interestCloud, smallObject_Cloud_vector, bigObject_Cloud_vector);
        // 클러스터의 크기 및 방향 측정 및 마커 생성 (필요 시 주석 해제)
